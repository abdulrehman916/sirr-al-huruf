 ...
 *
 * Shows: lock icon, feature name, "This feature is locked.", configured price,
 * WhatsApp Contact button, Redeem Access Code button, and a Back button.
 *
 * Price is fetched from PageVisibilityConfig (feature path → page path fallback).
 */
...
export default function FeatureLockedCard({ pagePath, featureId, featureLabel, onBack, onUnlocked }) {
  ...
  // Feature path for price lookup: e.g. /abjad/abjad_kabir
  const featurePath = `${pagePath}/${featureId}`;

  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Try cache first
      const cacheKey = `feature_price:${featurePath}`;
      const cached = getCached(cacheKey);
      if (cached !== null && cached !== undefined) {
        if (!cancelled) { setPrice(cached); setPriceLoading(false); }
        return;
      }

      try {
        // 1. Try feature-specific path
        let configs = await base44.entities.PageVisibilityConfig.filter(
          { page_path: featurePath, archived: false }, null, 1
        );
        let foundPrice = configs.length > 0 ? configs[0].price : null;

        // 2. Fallback to page-level path
        if (!foundPrice) {
          configs = await base44.entities.PageVisibilityConfig.filter(
            { page_path: pagePath, archived: false }, null, 1
          );
          foundPrice = configs.length > 0 ? configs[0].price : null;
        }

        if (!cancelled) {
          setPrice(foundPrice);
          setPriceLoading(false);
          setCached(cacheKey, foundPrice);
        }
      } catch {
        if (!cancelled) {
          setPrice(null);
  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl border p-8 text-center" style={{
        ...
        <p className="font-inter text-sm text-white/60 font-semibold mb-1">
