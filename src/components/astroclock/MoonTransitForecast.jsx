// ═══════════════════════════════════════════════════════════════
// MOON TRANSIT FORECAST — LIVE ASTRONOMY
// Shows current and upcoming Moon sign and mansion transitions
...
import { calculateMoonPosition, calculateMoonTransits } from "@/lib/astroClockMoonPosition.js";
...
export default function MoonTransitForecast() {
  const { isMalayalam } = useAstroClockLanguage();
  const [currentPosition, setCurrentPosition] = useState(null);
  const [signTransits, setSignTransits] = useState([]);
  const [mansionTransits, setMansionTransits] = useState(null);
  ...
  function updateMoonData() {
    ...
    setSignTransits(transits.signTransits || []);
    setMansionTransits(transits.mansionTransits || null);
    setLoading(false);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <MoonSignTransitsCard
        currentPosition={currentPosition}
        signTransits={signTransits}
        isMalayalam={isMalayalam}
      />

      {/* Lunar Mansion Transits Card */}
      <LunarMansionTransitsCard
        currentPosition={currentPosition}
        mansionTransits={mansionTransits}
        isMalayalam={isMalayalam}
      />
    </div>
  );
...
function MoonSignTransitsCard({ currentPosition, signTransits, isMalayalam }) {
          ...
          {currentSign && (
            <>
              ...
              <p className="font-inter text-xs" style={{ color: G.dim }}>
                {currentPosition?.longitude}° {isMalayalam ? "ദൈർഘ്യം" : "longitude"}
              </p>
            </>
          )}
        </div>
...
function LunarMansionTransitsCard({ currentPosition, mansionTransits, isMalayalam }) {
  const currentMansion = mansionTransits?.current;
  const nextMansion = mansionTransits?.next;
    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-5 rounded-xl border" style={{ background: G.bg, borderColor: G.border }}>
          ...
          {currentMansion && (
            <>
              <p className="font-amiri text-3xl font-bold mb-2 text-right" style={{ color: G.text }} dir="rtl">{currentMansion.arabic}</p>
              <p className="font-malayalam-md font-bold text-white">{currentMansion.name}</p>
              <p className="font-inter text-xs" style={{ color: G.dim }}>
                {isMalayalam ? "നമ്പർ" : "No."} {currentMansion.number}
              </p>
            </>
          )}
        </div>
        ...
        <div className="p-5 rounded-xl border" style={{ background: G.bgHi, borderColor: G.borderHi }}>
          ...
          {nextMansion && (
            <>
              <p className="font-amiri text-3xl font-bold mb-2 text-right" style={{ color: G.text }} dir="rtl">{nextMansion.arabic}</p>
              <p className="font-malayalam-md font-bold text-white">{nextMansion.name}</p>
              <p className="font-inter text-xs" style={{ color: G.dim }}>
      ...
      {currentMansion && (
        <div className="mb-6 p-4 rounded-xl" style={{ background: "rgba(212,175,55,0.05)", border: `1px solid ${G.faint}` }}>
          <div className="flex items-center justify-between">
            ...
            <p className="font-malayalam-md font-bold text-white">{formatDuration(currentMansion.remainingTime)}</p>
          </div>
        </div>
      )}
