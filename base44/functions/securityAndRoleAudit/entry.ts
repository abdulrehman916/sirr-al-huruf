...
Deno.serve(async (req) => {
  try {
    ...
    // Check code redemption security
    const codeSecurity = {
    ...
    const dbQueryAudit = {
      ...
      findings: [
        ...
        { check: "Service role usage", status: "APPROPRIATE", details: "asServiceRole used only in admin functions (redeemAccessCode, checkPageAccessFast)" },
    ...
    const expiredStatusIssue = {
      ...
      severity: "LOW",
      finding: "ApprovedUsersTab references EXPIRED status but UserAccessProfile enum has EXPIRED",
      location: "UserAccessProfile.json vs ApprovedUsersTab.jsx",
      impact: "Status mismatch could cause incorrect user filtering",
      recommendation: "Ensure EXPIRED status is consistently used across all components",
      status: "IDENTIFIED"
    };
    auditResults.security_issues.push(expiredStatusIssue);

    // Issue 3: Permission cache not flushed on admin status change
    const cacheFlushIssue = {
      category: "Permission Cache Flush Gap",
      severity: "LOW",
      finding: "When admin updates user permissions, cache is not explicitly flushed",
      location: "grantPagePermission.ts, revokePagePermission.ts",
      impact: "Users may see stale permissions for up to 30 seconds",
      recommendation: "Add cache flush calls after permission grant/revoke operations",
      status: "IDENTIFIED"
    };
    auditResults.security_issues.push(cacheFlushIssue);

    // ========== SUMMARY ==========
    auditResults.summary = {
      total_issues_found: auditResults.security_issues.filter(i => i.severity === "MEDIUM" || i.severity === "HIGH").length,
      critical_issues: 0,
      medium_issues: 1,
      low_issues: 2,
      secure_components: 8,
      overall_status: "MOSTLY_SECURE_WITH_MINOR_GAPS",
      immediate_actions_required: [
        "Add REMOVED status check in checkPageAccessFast",
        "Implement server-side pagination for user lists",
        "Add cache flush on permission changes"
      ],
      scalability_readiness: "READY_FOR_10K_USERS; NEEDS_PAGINATION_FOR_10M"
    };
